...
class Web {
  ...
  LoginSTS(args) {
    if (args.code) {
      return this.makeCall('LoginSTSWithCode', args)
        .then(res => {
          storage.setItem('token', `${res.token}`)
          return res
        })
        .catch(err => {
          if (err.message && err.message.includes('unknown method')) {
            throw new Error("サーバーが認可コードフローをサポートしていません。MinIOを最新バージョンにアップグレードしてください。")
          }
          throw err
        })
    }
